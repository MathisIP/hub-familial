...
- [lib/themes.ts](lib/themes.ts) — les **9 thèmes** (🌸 Rose défaut … 🌙 Nuit) en tokens CSS. Valeurs hex reprises telles quelles de `Contexte/Budget/00_Constantes.gs` : **ne pas les retoucher**, c'est la charte partagée avec les Sheets. `cssDesThemes()` génère le CSS de tous les thèmes.
...
- Écriture du maître en colonnes A→J uniquement (K AgendaID préservée). Statuts/Types lus dans Paramètres (A types, B statuts).
...
**Accueil refondu** (21/07/2026) : résumé des comptes (hors épargne) via `chargerAccueilBudget()` (lecture légère : soldes + params de saisie en 1 aller-retour ; exclut les comptes dont le nom contient « épargne »), rangée d'actions rapides, grille des modules, section Courses. Le bandeau d'état de connexion (échafaudage) a été retiré de l'accueil.
...
- **Bouton « Importer dans Drive »** ([ImportDrive.tsx](components/ImportDrive.tsx)) : **ouvre** le dossier « À classer » dans Drive (URL depuis `DRIVE_A_CLASSER_URL` en `.env`, passée par la page serveur). **Pas d'upload in-app** : le compte de service ne peut pas téléverser dans un Drive personnel (quota 0 → il ne peut posséder aucun fichier), et l'API Drive n'est pas activée dans le projet GCP `hub-familial-app`. Un vrai téléversement depuis l'app nécessiterait l'**OAuth utilisateur** (chantier séparé, aligné avec la direction multi-foyer). Si `DRIVE_A_CLASSER_URL` est absente, le bouton affiche une aide de config.
